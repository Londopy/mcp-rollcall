...
Claude Code reports a server as "failed to connect" or "Connection closed" and keeps
the reason to itself. This reads every place a server can be configured, checks each
one the way the harness would try to start it, and - on request - actually starts it,
does the MCP handshake, and shows the stderr and the tool list the harness hides.
...
def expand(value: str, env: dict, unset: set[str]) -> str:
    def sub(m):
...
def make_server(name: str, cfg: dict, scope: str, source: str) -> Server:
    s = Server(name=name, scope=scope, source=source)
    if not isinstance(cfg, dict):
        s.add("error", "config", "entry is not an object")
        return s
    s.type = str(cfg.get("type") or ("http" if cfg.get("url") and not cfg.get("command") else "stdio")).lower()
    s.command = str(cfg.get("command") or "")
    s.args = [str(a) for a in (cfg.get("args") or [])]
    s.env = {str(k): str(v) for k, v in (cfg.get("env") or {}).items()}
    s.url = str(cfg.get("url") or "")
    s.headers = {str(k): str(v) for k, v in (cfg.get("headers") or {}).items()}
    return s


def project_dir(start: Path) -> Path:
    """Nearest of cwd and its parents with a .mcp.json or .claude/, stopping at a git root
    and never climbing into the home directory (whose .claude/ is user config, not a project)."""
    start = start.resolve()
    home = Path.home().resolve()
    for p in [start, *start.parents]:
        if p == home and p != start:
            break
        if (p / ".mcp.json").exists() or (p / ".claude").is_dir():
            return p
        if (p / ".git").exists():
            return p
    return start
...
def load_all(a) -> Report:
    ...
    claude_json = Path(a.claude_json) if a.claude_json else Path.home() / ".claude.json"
    ...
    if cj.get("mcpServers"):
        rep.sources.append(f"user: {claude_json}")
    ...
    if pentry.get("mcpServers"):
        rep.sources.append(f"local: {claude_json} projects[...]")

    # settings that gate project servers and allow/deny lists
    settings = merged_settings(proj, home, Path(a.managed_dir) if a.managed_dir else MANAGED_DIRS.get(sys.platform, MANAGED_DIRS["linux"]))
    ...
    if pj.get("mcpServers"):
        rep.sources.append(f"project: {mcp_json}")
    ...
    cache, _ = read_json(home / "mcp-needs-auth-cache.json")
...
def check_static(s: Server, do_net: bool) -> None:
    if s.state in ("shadowed", "disabled", "denied"):
        return
    unset: set[str] = set()
    env = dict(os.environ)
    for k, v in s.env.items():
        ev = expand(v, env, unset)
        if not ev.strip():
    ...
    cmd = expand(s.command, env, unset)
    args = [expand(x, env, unset) for x in s.args]
    url = expand(s.url, env, unset)
    ...
    if s.type == "stdio":
        ...
        if resolved is None:
            ...
            s.add("warn", "windows-shim", f"bare '{cmd}' on Windows: the harness needs 'cmd /c {cmd} ...' or the full path to {stem}.cmd",
...
def probe(s: Server, timeout: float, cwd: Path) -> dict:
    ...
    unset: set[str] = set()
    env = dict(os.environ)
    env.update({k: expand(v, env, unset) for k, v in s.env.items()})
    cmd = [expand(s.command, env, unset), *[expand(x, env, unset) for x in s.args]]
    t0 = time.monotonic()
    try:
...
def render(rep: Report, a) -> None:
    ...
    scopes = ", ".join(f"{n} {k}" for k, n in sorted(counts.items(), key=lambda kv: RANK[kv[0]]))
    ...
    if not a.problems_only:
        ...
        hdr = f"  {'name':14} {'scope':8} {'type':6} {'command / url':{W}}"
        ...
        for s in rep.servers:
            ...
            row = f"  {s.name:14} {s.scope:8} {s.type:6} {short(cmdline(s), W):{W}}"
            if probed:
                p = s.probe or {}
